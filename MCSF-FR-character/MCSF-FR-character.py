...
#
#     # 保存最佳超参数到文件
#     with open('CRIbest_params.json', 'w') as f:
...
#
#     # 保存模型和scaler
#     torch.save(final_model.state_dict(), 'final_model_rgb_ratio1.pth')
#     with open('final_scaler_rgb_ratio1.pkl', 'wb') as f:
#         pickle.dump(full_train_set.scaler, f)
#     print("\n模型和标准化参数已保存至 final_model_rgb_ratio1.pth 和 final_scaler_rgb_ratio1.pkl")
#
#     # 测试集评估
...
# 1. 加载最佳超参数
with open('MCSF-FR-character.json', 'r') as f:
...
# 3. 加载模型权重
model.load_state_dict(torch.load('MCSF-FR-character.pth'))
model.eval()
print("模型权重加载成功。")
...
# 5. 加载标准化器
with open('MCSF-FR-character.pkl', 'rb') as f:
    scaler = pickle.load(f)
print("标准化器加载成功。")

# 6. 加载测试数据
test_df = pd.read_csv('C://Users//25726//Desktop//光谱液相//CNN//消融试验//特征外部.csv', header=None)

# 7. 提取特征（与训练时完全一致，特征波段：5个）
X_test_spec = test_df.iloc[:, :5].values.astype(np.float32)

test_total_carotenoid = test_df.iloc[:, 9].values.astype(np.float32)
test_red_index = calculate_rgb_ratio(test_df, 5, 6, 7)
X_test_chem = np.column_stack([test_total_carotenoid, test_red_index])
...
with torch.no_grad():
    # GPU预热
    dummy_spec = torch.randn(1, X_test_spec.shape[1]).to(device)  # 特征波段：5个
