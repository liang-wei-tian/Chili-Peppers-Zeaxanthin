...
#     X_train_chem = train_df.iloc[:, [23, 24]].values.astype(np.float32)  # 辣椒红和总类
#     y_train = train_df.iloc[:, 25].values.astype(np.float32)  # 玉米黄质
#
#     X_test_spec = test_df.iloc[:, :20].values.astype(np.float32)
#     X_test_chem = test_df.iloc[:, [23, 24]].values.astype(np.float32)
...
#     print("\n模型和标准化参数已保存至 final_model.pth 和 final_scaler.pkl")
#
#     # 测试集评估
...
# 1. 加载最佳超参数
with open('MCSFALL_model.json', 'r') as f:
...
# 3. 加载模型权重
model.load_state_dict(torch.load('MCSFALL_model.pth'))
model.eval()
print("模型权重加载成功。")
...
# 5. 加载标准化器
with open('MCSFALL_model.pkl', 'rb') as f:
